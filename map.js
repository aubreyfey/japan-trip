(() => {
  ...
  const render = () => {
    ...
    document.querySelector('#map-filters').innerHTML = [['day','Active day'],['Tokyo','Tokyo'],['Nagoya','Nagoya'],['Kyoto','Kyoto']].map(([value,label]) => `<button class="filter ${mode === value ? 'active' : ''}" data-map-mode="${value}">${label}</button>`).join('');
    board.innerHTML = `<span class="map-city tokyo">Tokyo</span><span class="map-city nagoya">Nagoya</span><span class="map-city kyoto">Kyoto</span>${visible.map((place,index) => `<button class="map-pin ${place.city.toLowerCase()}" style="left:${18 + (index * 17) % 68}%;top:${24 + (index * 23) % 54}%" data-map-place="${place.name}" aria-label="View ${place.name}"><b>●</b><span>${place.name}</span></button>`).join('')}`;
