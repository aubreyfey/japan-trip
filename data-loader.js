...
async function loadTripContent() {
  const files = ['attractions','itinerary','budget','hotels','restaurants','experiences','optional','media'];
  ...
  const media = content.media?.destinations || {};
  places.splice(0, places.length, ...content.attractions.map(place => ({
    ...
    image: media[place.name]?.hero?.url || `https://images.unsplash.com/${place.image}?auto=format&fit=crop&w=900&q=78`
  })));
